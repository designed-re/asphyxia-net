...
namespace Proxy.Formatters
{
    public class EamuseXrpcInputFormatter : InputFormatter
    {
        ...
        public async Task<InputFormatterResult> TestRead(HttpContext context)
        {
            var request = context.Request;

            if (!context.Request.Headers.TryGetValue("X-Compress", out var header))
                return await InputFormatterResult.FailureAsync();

            string compAlgo = header.ToString();

            string eAmuseInfo = null;
            if (context.Request.Headers.TryGetValue("X-Eamuse-Info", out header))
                eAmuseInfo = header.ToString();
